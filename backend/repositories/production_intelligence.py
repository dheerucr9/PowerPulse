...
class ProductionIntelligenceRepository:
    ...
    def fetch_site_bucket_history(self, bucket_of_day: int, start_ts: int, end_ts: int) -> list[dict[str, Any]]:
        rows = self._executor.execute(
            text(
                """
                SELECT ts, production_kw
    ...
    def fetch_panel_ids(self) -> list[str]:
        rows = self._executor.execute(
            text(
                """
        ...
        return [str(row["panel_id"]) for row in rows]

    def fetch_panel_bucket_history(
        self,
        panel_id: str,
        bucket_of_day: int,
        start_ts: int,
    ...
    def fetch_panel_samples_at_ts(self, ts: int) -> list[dict[str, Any]]:
        rows = self._executor.execute(
            text(
                """
                SELECT panel_id, p_kw, state
                FROM samples_raw
                WHERE ts = :ts
                ORDER BY panel_id ASC
                """
    ...
    def upsert_panel_baseline(
        ...
        *,
        ...
        self._executor.execute(
            text(
                """
                INSERT INTO panel_power_baselines
                (
                    site_id,
                    panel_id,
                    metric,
                ...
                ON CONFLICT (site_id, panel_id, metric, bucket_start_ts, bucket_granularity_seconds)
                DO UPDATE SET
                    baseline_kw = excluded.baseline_kw,
                    baseline_stddev_kw = excluded.baseline_stddev_kw,
                    sample_count = excluded.sample_count,
    ...
    def fetch_existing_site_underproduction_anomaly(self, site_id: str, sample_ts: int) -> dict[str, Any] | None:
        row = self._executor.execute(
            text(
                """
                SELECT anomaly_id
                ...
                WHERE site_id = :site_id
                  ...
                  AND metric = 'production_kw'
                  AND sample_ts = :sample_ts
                  AND direction = 'below'
                LIMIT 1
                """
            ),
            {"site_id": site_id, "sample_ts": sample_ts},
        ).mappings().first()
        return dict(row) if row else None
